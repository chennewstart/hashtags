...
public class DStateUpdateQuery implements QueryFunction<State, Long> {
	...
	public List<Long> batchRetrieve(State dState, List<TridentTuple> tuples) {
		...
		for (TridentTuple tuple : tuples) {
			if (val == null) {
				ds.add((long) tuples.size());
			} else {
				ds.add(val + tuples.size());
			}
		}
		((MemoryMapState<Long>) dState).multiPut(keys, ds);
		return ds;
	}
